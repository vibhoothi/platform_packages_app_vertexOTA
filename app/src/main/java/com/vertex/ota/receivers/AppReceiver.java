...
package com.vertex.ota.receivers;
...
import com.vertex.ota.R;
import com.vertex.ota.RomUpdate;
import com.vertex.ota.activities.AddonActivity;
import com.vertex.ota.activities.AvailableActivity;
import com.vertex.ota.tasks.LoadUpdateManifest;
import com.vertex.ota.utils.Constants;
import com.vertex.ota.utils.Preferences;
import com.vertex.ota.utils.Utils;
...
public class AppReceiver extends BroadcastReceiver implements Constants{
	...
	public void onReceive(Context context, Intent intent) {
		...
		if (action.equals(DownloadManager.ACTION_DOWNLOAD_COMPLETE)) {
			...
			Set<Integer> set = vertexOTA.getAddonDownloadKeySet();
			Iterator<Integer> iterator = set.iterator();

			while (iterator.hasNext() && isAddonDownload != true) {
				int nextValue = iterator.next();
				if (id == vertexOTA.getAddonDownload(nextValue)) {
			...
			if (isAddonDownload) {
				...
				if (DownloadManager.STATUS_SUCCESSFUL != cursor.getInt(statusIndex)) {
					...
					vertexOTA.removeAddonDownload(keyForAddonDownload);
